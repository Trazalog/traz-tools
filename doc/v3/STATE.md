...
|---|---|---|
| 2026-08-08 | **Corrección en §7: SÍ hace falta desplegar `traz-comp-dnato` en el servidor de Dnato de GCP.** Rodolfo repreguntó tras ver commits de identidad en GitHub que la sección anterior decía que no hacían falta. Verificado directamente en `traz-comp-dnato` (clon local): los commits de JWT/OAuth 2.1 PKCE/JWKS (E9-IDENT-03/04) están SOLO en `develop-v3` de ese repo, no en `develop` ni `master` — si el servidor de Dnato de ese proyecto GCP corre otra rama, no tiene ese código. Agregado §7.0-bis con el checklist real (deploy de `develop-v3`, generar claves RS256 en ese server, migración DB, `oauth_clients.php`) — trabajo de `traz-comp-dnato`, solo señalado acá como dependencia | `doc/v3/deployment-gcp.md` §7.0-bis |
| 2026-08-08 | **Checklist de identidad para GCP (E7-INFRA-03), a pedido directo de Rodolfo** tras preguntar si desplegar `develop-v3` al servidor de Dnato traía consigo los cambios de `apim-keymanager-dnato.md`. Nueva §7 en `deployment-gcp.md` con el checklist adaptado a la VM de GCP (corregido el mismo día, ver fila de arriba) | `doc/v3/deployment-gcp.md` §7 |
