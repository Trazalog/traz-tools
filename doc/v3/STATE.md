...
**Última actualización:** 2026-08-08 por Claude Code (smoke test real del MCP Server publicado — 2 bugs de config encontrados y corregidos)

### Tareas activas

| ID | Descripción | Clase | Estado | Rama / PR |
|---|---|---|---|---|
| — | Smoke test real de `Trazalog MCP Server` contra el gateway APIM (:8243) + fix de paths en docs | 🟡 | **Completada.** 9/9 tools confirmadas funcionando end-to-end contra `https://localhost:8243/trazalog/mcp/1.0/mcp` con JWT real de Dnato (vía `mint-dnato-jwt.py` + `dnato-jwks-server.py`). Aislamiento confirmado (empr_id=187 no ve datos de empr_id=1). Escrituras confirmadas con Bonita real (OT 292/case 30003, pedido 1482/case 30004, descartables). En el camino, 2 bugs reales de configuración bloqueaban todo: (1) la app `TrazalogDnatoMCP` no tenía suscripción al MCP Server nuevo → `900908`; (2) el `endpointConfig` del MCP Server estaba vacío y el de la API apuntaba a `10.142.0.13:8280` (dato incorrecto mío desde la Tarea 3.4) en vez de `localhost:8290` → `404`. Ambos corregidos en vivo contra el APIM de Rodolfo (REST API, con revisiones nuevas desplegadas) y documentados en `virtual-mcp-unificado.md` (§2.3, §2.8-bis nuevo, tabla de troubleshooting en §4) para que no se repitan. `x-mcp-annotations` confirmado que NO aparece en `tools/list` — gap preexistente de Sprint 2, no bloqueante | `fix/mcp-server-real-paths` — PR abierto, sin mergear |
| — | Fix `virtual-mcp-unificado.md` §2.1/§2.4 — instrucción de Key Manager Dnato incorrecta | 🟡 | Completada | `fix/mcp-unificado-doc-key-manager-dnato` — PR abierto, sin mergear |
