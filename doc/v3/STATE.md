...
**Última actualización:** 2026-08-08 por Claude Code (paths reales del MCP Server publicado)

### Tareas activas

| ID | Descripción | Clase | Estado | Rama / PR |
|---|---|---|---|---|
| — | Actualizar docs/ADR con los paths reales del MCP Server publicado (`/trazalog/mcp`, no `/trazalog-operaciones`) | 🟢 | **Completada.** Rodolfo publicó la API/MCP Server con Name `Trazalog MCP Server`, contexto `/trazalog/mcp`, versión `1.0` (endpoint confirmado: `https://<host>:8243/trazalog/mcp/1.0/mcp`) — distinto de la sugerencia original de ADR-013 (`trazalog-operaciones`). Actualizado `trazalog-operaciones.yaml` (servers), `virtual-mcp-unificado.md` (§2.2, §2.6, §2.9, §3, §4) y la pregunta abierta de ADR-013 marcada como resuelta. Los tests Hurl (`tests/security/mcp-facade-*.hurl`) NO necesitaron cambios — apuntan al MI directo (`/tools/mcp/...`), capa que no se tocó | `fix/mcp-server-real-paths` — PR abierto, sin mergear |
