...
| H-003 | 2026-08-19 | DocTest F1 | traz-comp-dnato + BD | deuda | 🟡 | El superusuario está definido **en dos lugares distintos y desacoplados**: `TOOLS_ADMIN_USER` en `constants.php` (controla el menú de Gestión de Empresas) y `core.tablas.coresuper_admin` (lo usa el trigger de alta de empresa, con `jperez@prueba.com` como default hardcodeado). Si no coinciden, el sistema tiene dos "superusuarios" con poderes distintos | `constants.php:105`, `configuracion_inicial_empresa_trg_func.sql:15-25` | abierto |
