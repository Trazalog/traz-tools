		...
		function infoproceso($tarea){
			...
			switch ($processId) {
				case BPM_PROCESS_ID_PEDIDOS_NORMALES:
					log_message('DEBUG','#TRAZA|INFOPROCESO_HELPER|/solicitudContenedores/info/.$case_id): $case_id >> '.json_encode($case_id));
				...
				case BPM_PROCESS_ID_PEDIDOS_EXTRAORDINARIOS:
					log_message('DEBUG','#TRAZA|INFOPROCESO_HELPER|/solicitudRetiro/proceso/retiro/case/.$case_id): $case_id >> '.json_encode($case_id));
				...
				case BPM_PROCESS_ID_REPARACION_NEUMATICOS:
					log_message('DEBUG','#TRAZA|INFOPROCESO_HELPER|REPARACION_NEUMATICOS/".$case_id : $case_id >> '.json_encode($case_id));
			...
			case BPM_PROCESS_ID_SOLICITUD_CONTENEDORES:
					...
					log_message('DEBUG','#TRAZA|INFOPROCESO_HELPER|/solicitudContenedores/info/.$case_id): $case_id >> '.json_encode($case_id));
					$aux = $ci->rest->callAPI("GET",REST_RESI."/solicitudContenedores/info/".$case_id);
			...
			case BPM_PROCESS_ID_SOLICITUD_RETIRO_CONTENEDORES:
					...
					log_message('DEBUG','#TRAZA|INFOPROCESO_HELPER|/solicitudRetiro/proceso/retiro/case/.$case_id): $case_id >> '.json_encode($case_id));
					$aux = $ci->rest->callAPI("GET",REST_RESI."/solicitudRetiro/proceso/retiro/case/".$case_id);
					$data = json_decode($aux["data"]);
					$aux = $data->solicitud_retiro;						
				break;
			...
			case BPM_PROCESS_ID_ORDEN_TRANSPORTE:
					...
					log_message('DEBUG','#TRAZA|INFOPROCESO_HELPER|/ordenTransporte/info/entrega/case/".$case_id : $case_id >> '.json_encode($case_id));

					$aux = $ci->rest->callAPI("GET",REST_RESI."/ordenTransporte/info/entrega/case/".$case_id);
					$data =json_decode($aux["data"]);
				...
				default:
					log_message('DEBUG','#TRAZA|INFOPROCESO_HELPER|SEIN ALM PAN TAR/".$case_id : $case_id >> '.json_encode($case_id));
			...
			}  elseif (BPM_PROCESS_ID_SOLICITUD_CONTENEDORES == $processId) {
				$nombreProceso = 'Solicitud de Contenedores';
			}  elseif (BPM_PROCESS_ID_SOLICITUD_RETIRO_CONTENEDORES == $processId) {
				$nombreProceso = 'Solicitud de Retiro de Contenedores';
			}  elseif (BPM_PROCESS_ID_ORDEN_TRANSPORTE == $processId) {
				$nombreProceso = ' - Orden Transporte Nº: '.$aux->ortr_id;
			}else{
				$nombreProceso = 'Estandar';
			}
					...
					?>
								...
								<form class="formNombre1" id="IDnombre">
									<div class="col-md-12">
										...
										<div class="col-md-6">
											<div class="form-group">
												<label for="domicilio" name="">Justificación:</label>
						...
						?>
								...
								<form class="formNombre1" id="IDnombre">  
									<div class="col-md-12">
											...
											<div class="col-md-6">
												<div class="form-group">
														<label for="domicilio" name="">Observación:</label>
