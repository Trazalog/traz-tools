...
function frmGuardar(e,accionPosGuardado = false) {
	wo();
		...
		$.ajax({
			...
			success: function (rsp) {
				$("#" + form)
					...
					if(accionPosGuardado)accionPosGuardado()
  ...
  Object.keys(data).forEach(e => {
      ...
      switch (obj.getAttribute('type')) {
		...
		if ( dato !='') {
			...
			$(obj).addClass('hidden');
				$(obj).closest('.form-group').find('.help-block').html(`<a download='documento.png' href='data:image/png;base64,${decodificado}' id='descarga_documento'>Click para descargar el puto archivo</a>`);
		} else{
			$(obj).addClass('hidden');
			$(obj).closest('.form-group').find('.help-block').html(`<a href='${data[e]}' id='descarga_documento'>Click para descargar el puto archivo</a>`);
