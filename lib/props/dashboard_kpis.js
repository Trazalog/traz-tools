...
var TZDash = (function () {
  ...
  function showKpis(cont, base, emprId) {
    ...
    // redibuja al cambiar tamaño (canvas es sensible al ancho) — una sola vez
    if (!resizeWired) {
      resizeWired = true;
      var rt;
      window.addEventListener('resize', function () {
        var c = document.querySelector('.tzdash .tz-kpis');
        if (!c) return;
        clearTimeout(rt); rt = setTimeout(function () { redrawAll(c, base); }, 300);
      });
    }
  }

  // ¿el usuario ya "comenzó el día" hoy? (per navegador). Sin localStorage: no molestar (true).
  function yaComenzoHoy(hoy) {
    try { return localStorage.getItem('tzdash_started') === hoy; } catch (e) { return true; }
  }

  function init(base, emprId, hoy) {
    var cont = document.querySelector('.tzdash .tz-kpis');
    if (!cont) return;
    var wrap = document.getElementById('tz-kpis-wrap');
    var startEl = document.querySelector('.tzdash .tz-start');
    var btn = document.querySelector('.tzdash .tz-start-btn');

    // Ya comenzó hoy (o falta soporte): se muestra el tablero tal cual.
    if (!wrap || !startEl || !btn || yaComenzoHoy(hoy)) {
      showKpis(cont, base, emprId);
      return;
    }

    // Primera vez del día: ocultar los KPIs y mostrar "¿Comenzamos el día?" en el saludo.
    wrap.hidden = true;
    startEl.hidden = false;
    btn.addEventListener('click', function () {
      try { localStorage.setItem('tzdash_started', hoy); } catch (e) {}
      startEl.hidden = true;
      wrap.hidden = false;
      wrap.classList.add('tz-reveal');       // fade-in difuminado
      showKpis(cont, base, emprId);
    });
  }
